...
public class IatDemo extends Activity implements OnClickListener {
	...
	public void onClick(View view) {
		...
		}else if (view.getId() == R.id.iat_recognize) {// 开始听写
		...
		}else if (view.getId() == R.id.iat_stop) { // 停止听写
			mIat.stopListening();
			showTip("停止听写");
		}else if (view.getId() == R.id.iat_cancel) {// 取消听写
			mIat.cancel();
			showTip("取消听写");
		}else if (view.getId() == R.id.iat_upload_contacts) {// 上传联系人
			showTip(getString(R.string.text_upload_contacts));
			ContactManager mgr = ContactManager.createManager(IatDemo.this,
					mContactListener);
			mgr.asyncQueryAllContactsName();
		}else if (view.getId() == R.id.iat_upload_userwords) {// 上传用户词表
